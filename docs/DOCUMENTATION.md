...
| FR-29 | 検索除外の固定ページ: 固定ページに `noindex` を指定すると、`robots`メタとsitemap除外で検索結果から外れる | `src/content/pages/`, `src/content.config.ts`, `src/pages/[slug].astro`, `astro.config.mjs`, `public/admin/config.yml` | 用途はGmail OAuth同意画面から参照するアプリ案内・プライバシーポリシー。CMSから編集できる |
...
| FR-29 | 検索除外の固定ページ | build / E2E app-info | 2.5.9章 #1〜#5、E-46: 2ページ×3デバイス | 実クリック・axe | 充足 |
