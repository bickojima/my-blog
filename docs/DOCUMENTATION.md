...
| 1.53 | 2026-09-09 | FR-29追加。個人用Gmailアプリの最小限の紹介・プライバシーポリシー、QA、実操作E2Eを追加。両ページは`noindex`＋sitemap除外で検索結果に出さない。Vitest 588→592件。staging先行 |
...
[要件確認QA・判断経緯](qa-2026-09-09-otp-app-pages.md)。ユーザー指定の既存ドメインを利用し、公開文面は超最小限とする。専用Markdownルートを `AppInfo.astro` → `Base.astro` で描画する。Gmail認証やバックエンド処理は追加しない。Googleへの登録はmain公開後に行う。ページはOAuth同意画面から参照されるだけで検索流入を想定しないため、`Base.astro` の `noindex` プロパティと `astro.config.mjs` の sitemap `filter` で検索結果から外す。Google側の要件は「公開アクセスできること」であり、`noindex` は登録の妨げにならない。
...
| FR-29 | 個人用アプリ案内: ログイン不要の最小限の紹介とGmailプライバシーポリシーを同一ドメインに公開し相互リンクする | `src/pages/playwright-home/`, `src/layouts/AppInfo.astro`, `astro.config.mjs` | ブログ記事・CMSナビとは別。検索結果には出さない（`noindex`＋sitemap除外）。確認項目はQA記録 |
...
| FR-29 | 個人用アプリ案内・データ取扱い表示 | build / E2E app-info | 2.5.9章 #1〜#4、E-46: 2ページ×3デバイス | 実クリック・Enter・axe | 充足 |
...
| 指標 | 目標値 | 現状 |
|:---|:---|:---|
| Vitestテスト全PASS | 100% | 592/592 (100%) |
| Playwright E2Eテスト全PASS | 100% | 直近実行: 442 PASS・8 skip / 450件、flakyなし (100%) |
