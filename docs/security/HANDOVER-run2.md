...
| 4 | ゾンビテスト（ガードにより素通りする空テスト）が旧設計を主張し続けていた | `fuzz-validation.test.mjs:993` は実効化済み。**ただし下記「残作業1」を参照** |
...
### 1. 【要修正】2件目の空テスト（本セッションで新規発見・未対応）
`tests/fuzz-validation.test.mjs:1054`

```js
it('/* と /admin/* で同名ヘッダーが異なる値で重複していない（Bug #28 再発防止）', () => {
  ...
  if (globalVal && adminVal) {      // ← 設計変更後は adminVal が常に null
    expect(globalVal, ...).toBe(adminVal);
  }
});
```

`993` 行の空テストと**まったく同じ構造の問題**。設計変更により `adminVal` が全ヘッダーで `null` になるため、ループは回るが `expect` が一度も実行されない。`build.test.mjs:985` の `/* と /admin/* で同名ヘッダーが一切重複していない（SEC-30, Bug #49 再発防止）` と役割が重複しているため、**実効的な検証への書き換えか、重複解消のうえ統合**が必要。削除・スキップは禁止。

### 2. 【未着手】CLAUDE.md のテスト件数更新
- `CLAUDE.md:26` — `npm test` の説明「622テスト」→ 実測値 **624**
- `CLAUDE.md:111` — 「Vitest ...（622テスト、記事数により変動）」→ **624**
- `CLAUDE.md:248` — 「現行テスト定義はVitest 610件、E2E 453件」→ **Vitest を 624 に統一**（610 と 622 が既に矛盾していた。E2E 件数は今回未実測なので触らないこと）
- `AGENTS.md` の有無を確認し、テスト件数・セキュリティ要件の記述があれば整合させる
