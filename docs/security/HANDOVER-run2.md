...
### 6. 【一部完了】Issue の処理
| Issue | 状態（2026-09-21実測、`gh issue view` で確認） | 対応 |
|---|---|---|
| #114 | **closed** | SEC-29 / Bug #48 で修正済み。本番反映後にクローズ済み |
| #115 | **closed** | SEC-30 / Bug #49 で修正済み。staging / 本番で重複解消を実測しクローズ済み |
| #116 | **closed** | オーナーがダッシュボードで `npm ci` 使用を確認しクローズ済み |
| #117 | open | 項目4・項目11 のみ対応済み。**残りの項目（1〜3, 5〜10, 12〜15）は未対応**（うち項目2・12はSEC-33/SEC-34で追加対応済み、残りは引き続き未対応）。項目13（`decap-cms-app` 未使用）はトラック B で解消予定 |
