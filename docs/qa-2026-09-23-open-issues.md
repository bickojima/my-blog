...
- 指定DriveパスはNFDの `GoogleDrive-seima.tsuboi@gmail.com/マイドライブ/05_自動化環境/06_my-blog` として実在し、各フォルダを読み取れる。通常sandboxではwrite不可だったが、明示的に許可された昇格プローブで一意な小ファイルの作成・SHA-256読み戻し・同一ファイル削除に成功し、ファイル不在を確認した。したがって通常時のwrite不可はsandbox制限。全量コピーは未実施。ディスク空きは18GiB。
- `/Users/s-tsuboi/git/playwright-home` はローカルに存在し、関連資料・コードは読み取り済み。未追跡ファイルがあるためstatusのみ確認し、変更していない。`/Users/playwright/git/playwright-home`（Mac mini側）はこのMac上に存在せず、本番実機は未確認。
