...
状態: **要件定義確定・実作業中（2026-09-23 ユーザー決定をQ6〜Q9・Q13〜Q17に反映）。#81・#129はクローズ済み。#117（PR #135）・#128（PR #137）はstaging反映済み。#127/#130/#132はPR #134で実装中。#90/#131の履歴書換えは他issue完了・main反映・open PRゼロの後に凍結して実施する。**
...
| Q7 | Issue #128でCI失敗時にPagesデプロイを止める方式 | Cloudflare PagesのBuild commandは既に `npm run build`（Vitestゲート付き）であることをダッシュボードで確認した。Issueの前提（CIとPagesが独立でテストなしにdeployされる）は誤り。stagingに意図的な失敗コミットを入れる実証は不要（ユーザー判断）とし、ローカル実証（PR #137）で代える | 確認済み |
...
| Q9 | Issue #130のAnalytics CSPエラー | CSPは緩和しない（`static.cloudflareinsights.com` を許可リストへ追加しない）。CMS機能への影響がないことを実操作E2Eで確認中 | 確認済み（検証中） |
...
| [#127 環境固有ファイルのmerge汚染](https://github.com/bickojima/my-blog/issues/127) | issueはBug #51を報告。PR #125/#126本文はSEC-35テスト、main復元、staging検証を報告する一方、両方向merge時の予防策は未確定。ローカルremote refsでもmain/stagingの3ファイル差分を確認。 | 実装中・PR #134（Q6: ビルド時/実行時の環境値自動導出） | staging→main / main→staging双方の試行で3ファイルが正しい環境値を保つ。feature/PRで誤検知・開発阻害がない。テストだけでなく実merge結果を記録 |
| [#128 CI失敗でPages deployを止める](https://github.com/bickojima/my-blog/issues/128) | CIとPagesは独立というissue記載。達成条件は失敗コードを本番へ反映しないこと。 | PR #137でstaging反映済み。Pages Build commandが `npm run build`（Vitestゲート付き）であることをダッシュボードで確認し、ローカル実証で代える（Q7） | 採用方式を設定し、失敗するテストを使って本番deployが停止することを安全な環境で実証。PlaywrightをCIに追加しない既定方針との整合を確認 |
...
| [#130 AdminでCloudflare InsightsがCSPに遮断](https://github.com/bickojima/my-blog/issues/130) | 2026-09-23 本番/staging `/admin/` GETは200、両環境のCSPは一致。`static.cloudflareinsights.com` は許可されず意図通りブロックされる。ただしこれだけではCMS機能無影響の証拠にならない。 | 実装中・PR #134。CSPは緩和しない（Q9）。意図的遮断コメントとCMS実操作E2Eで確認中 | コメントを追加し、CSPにAnalyticsを許可していないことを静的確認。本番/staging adminでOAuth後の編集画面、保存/preview操作を実操作し、Analytics beacon以外のCSP違反がないことを確認。スクリーンショットと結果を保存し、レビュー後に理由を記録して閉じる |
