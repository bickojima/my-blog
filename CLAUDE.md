...
npm test             # Vitest 全テスト実行（634テスト、記事数により変動）
...
- **Vitest**: 設定検証、コンテンツ検証、単体テスト、ビルド統合テスト、セキュリティ検証、ファズテスト、基本機能保護テスト（634テスト、記事数により変動）。`.github/workflows/ci.yml` によりmain/staging/feature/*へのpush・PRで自動実行される（Playwright は含めない）
...
7. **Vitest だけでは main マージ不可**。ローカル `npm run test:e2e`（3デバイス全件）完了が必須。CI の Vitest 成功・staging CMS 実ログイン確認・「後で E2E」は代替にならない（Bug #50）
8. **CI に Playwright は載せない**（実行時間のためローカル運用を継続）。E2E 必須化はローカル手順と Vitest による手順文書の固定で行う
...
- 現行テスト定義はVitest 634件、E2E 453件（旧444件＋FR-29 9件、445実行+8スキップ）。QAは `docs/qa-2026-09-09-otp-app-pages.md`。
