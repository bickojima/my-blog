 ...
 * 使用方法:
 *   1. npm run build
 *   2. node evidence/2026-05-22/verify-cms19-month-filter.mjs
 */
...
const EVIDENCE_DIR = join(process.cwd(), 'test-results', 'evidence', 'cms19-month-filter');
...
async function runScenarios(page, deviceName) {
  ...
  // S2: 月セレクターによる年月フィルター
  {
    ...
    const state = await page.evaluate(() => {
      const selector = document.getElementById('cms-month-selector');
      ...
      const groups = Array.from(document.querySelectorAll('[class*="GroupHeading"]')).map(h => {
        ...
        return {
          ...
          visible: !!container && getComputedStyle(container).display !== 'none',
          inlineDisplay: container ? container.style.display : null,
        };
      });
      return {
        selected,
        activeFilter: selector ? selector.dataset.activeFilter : '',
        groups,
        visibleGroups: groups.filter(g => g.visible).map(g => g.heading),
        hiddenGroups: groups.filter(g => !g.visible).map(g => g.heading),
      };
    });

    const pass = !!state.selected
      && state.activeFilter === state.selected
      && state.visibleGroups.length === 1
      && state.visibleGroups[0] === state.selected
      && state.hiddenGroups.length >= 1;

    await addRedBorder(page, '#cms-month-selector', '選択中の年月');
    await addRedBorderByEval(page, (doc) => {
      const selected = doc.getElementById('cms-month-selector')?.value;
      return Array.from(doc.querySelectorAll('[class*="GroupHeading"]'))
        .find(h => (h.textContent || '').trim() === selected);
    }, '表示中のグループ');
    ...
    console.log(`    Visible: ${state.visibleGroups.join(' > ')}`);
    console.log(`    Hidden: ${state.hiddenGroups.join(' > ')}`);
    console.log(`    Result: ${pass ? 'PASS' : 'FAIL'}`);
...
<html lang="ja"><head><meta charset="UTF-8"><title>CMS-19 年月フィルター修正 エビデンスレポート (2026-05-22)</title>
<style>
...
<h1>CMS-19 年月フィルター修正 エビデンスレポート</h1>
<p>日付: 2026-05-22 | 要件: CMS-19（年月グルーピングUI改善） | Bug #37</p>
...
<ul>
  ...
  console.log('\n[REPORT] test-results/evidence/cms19-month-filter/report.html 生成完了');
}
...
async function main() {
  ...
  try {
    ...
    server = await startServer(join(process.cwd(), 'dist'));
