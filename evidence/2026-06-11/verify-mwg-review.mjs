/**
 * Modern Web Guidance レビュー指摘修正後の検証（2026-06-11）
 */
...
const OUT = join(process.cwd(), 'evidence/2026-06-11');
...
async function annotate(page, annotations) {
  await page.evaluate((items) => {
    document.querySelectorAll('[data-evidence-annotation]').forEach(el => el.remove());
    for (const item of items) {
      const target = document.querySelector(item.selector);
      if (!target) continue;
      const rect = target.getBoundingClientRect();
      const box = document.createElement('div');
      box.dataset.evidenceAnnotation = 'true';
      box.style.cssText = [
        'position:absolute',
      ...
      label.dataset.evidenceAnnotation = 'true';
      label.textContent = item.label;
      label.style.cssText = [
        'position:absolute',
        `left:${rect.left + window.scrollX}px`,
        `top:${Math.max(0, rect.top + window.scrollY - 25)}px`,
        'background:#e00000',
        'color:#fff',
...
async function runSite(browser, dev) {
  ...
  await annotate(page, [
    { selector: '.post-list', label: 'F-1: 7枚目以降のみ描画最適化' },
    { selector: '.post-thumbnail', label: 'F-10: width/height属性' },
  ]);
  await page.screenshot({ path: join(SS, `S01-top-${dev.name}.png`), fullPage: true });
  ...
  const violCards = s01.cardInfo.filter(c => c.aboveFold && c.cv === 'auto');
  ...
  if (await toggle.count() > 0) {
    ...
    await annotate(page, [
      { selector: '.nav-dropdown-toggle', label: 'F-2/F-3: Escape・44px' },
      { selector: '.nav-dropdown-menu', label: 'F-2: Tab離脱で閉じる' },
    ]);
    await page.screenshot({ path: join(SS, `S03-dropdown-open-${dev.name}.png`), fullPage: true });
  ...
  await annotate(page, [{ selector: '.post-content', label: 'F-8/F-9: 本文リンク・pre到達性' }]);
  await page.screenshot({ path: join(SS, `S04-post-${dev.name}.png`), fullPage: true });
  await page.evaluate(() => document.querySelectorAll('[data-evidence-annotation]').forEach(el => el.remove()));
...
async function runCms(browser, dev) {
  ...
  await annotate(page, [{ selector: '#cms-month-selector', label: 'F-4/F-5: 名前・44px' }]);
  await page.screenshot({ path: join(SS, `C01-list-${dev.name}.png`), fullPage: true });
  ...
  if (await sel.count() > 0 && await sel.isVisible()) {
    ...
    if (target) {
      ...
      await annotate(page, [
        { selector: '#cms-month-selector', label: '実操作: 年月選択' },
        { selector: '[class*="GroupHeading"]', label: '選択年月のみ表示' },
      ]);
      await page.screenshot({ path: join(SS, `C02-filtered-${dev.name}.png`), fullPage: true });
  ...
  if (await entry.count() > 0) {
    ...
    await annotate(page, [{ selector: '[class*="EditorControlBar"]', label: '認証後CMS編集画面' }]);
    await page.screenshot({ path: join(SS, `C04-editor-${dev.name}.png`), fullPage: true });
...
const postSource = readFileSync(join(process.cwd(), 'src/pages/posts/[year]/[month]/[slug].astro'), 'utf-8');
const pageSource = readFileSync(join(process.cwd(), 'src/pages/[slug].astro'), 'utf-8');
const pluginSource = readFileSync(join(process.cwd(), 'src/plugins/rehype-focusable-code-blocks.mjs'), 'utf-8');
record('Source', 'F08', '記事・固定ページ本文リンクに下線・識別色・focus-visible', [postSource, pageSource].every(s => s.includes('text-decoration: underline') && s.includes('color: #1a73e8') && s.includes(':global(a:focus-visible)')));
record('Source', 'F09', 'rehype pluginがpreへtabindex=0を付与', pluginSource.includes("node.tagName !== 'pre'") && pluginSource.includes('node.properties.tabIndex = 0'));
...
const screenshotRows = [
  ['トップページ', 'S01-top'],
  ['ナビドロップダウン', 'S03-dropdown-open'],
  ['記事本文', 'S04-post'],
  ['CMS記事一覧', 'C01-list'],
  ['CMS年月絞り込み', 'C02-filtered'],
  ['CMS記事編集', 'C04-editor'],
];
const report = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Modern Web Guidance 修正検証</title><style>
body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;margin:24px;color:#222}h1,h2{margin:0 0 16px}.summary{padding:12px 16px;background:${fails.length ? '#fff0f0' : '#eef9f0'};border-left:5px solid ${fails.length ? '#c00' : '#16803a'};margin-bottom:24px}.checks{border-collapse:collapse;width:100%;margin-bottom:32px}.checks th,.checks td{border:1px solid #ddd;padding:7px;text-align:left}.pass{color:#16803a;font-weight:700}.fail{color:#c00;font-weight:700}.grid{display:grid;grid-template-columns:repeat(3,minmax(260px,1fr));gap:14px;margin-bottom:36px}.shot{border:1px solid #ccc;padding:8px;background:#fafafa}.shot img{width:100%;height:auto;display:block}.shot strong{display:block;margin-bottom:6px}@media(max-width:900px){.grid{grid-template-columns:1fr}}
</style></head><body><h1>Modern Web Guidance レビュー指摘 修正検証</h1><div class="summary">${results.length} checks / ${fails.length} failures。赤枠は検証対象を示す。CMS画像はOAuth 3ステップハンドシェイクとGitHub APIモックによる認証後画面。</div>
<table class="checks"><thead><tr><th>Device</th><th>ID</th><th>Check</th><th>Result</th></tr></thead><tbody>${results.map(r => `<tr><td>${esc(r.device)}</td><td>${esc(r.id)}</td><td>${esc(r.name)}</td><td class="${r.pass === false ? 'fail' : r.pass === true ? 'pass' : ''}">${r.pass === false ? 'FAIL' : r.pass === true ? 'PASS' : 'INFO'}</td></tr>`).join('')}</tbody></table>
${screenshotRows.map(([title, prefix]) => `<h2>${title}</h2><div class="grid">${DEVICES_CONFIG.map(d => { const file = `${prefix}-${d.name}.png`; const exists = (() => { try { readFileSync(join(SS, file)); return true; } catch { return false; } })(); return exists ? `<div class="shot"><strong>${d.name}</strong><img src="screenshots/${file}" alt="${esc(title)} ${d.name}"></div>` : ''; }).join('')}</div>`).join('')}
</body></html>`;
writeFileSync(join(OUT, 'report.html'), report);
writeFileSync(join(OUT, 'work-completion-report.html'), `<!doctype html><html lang="ja"><head><meta charset="utf-8"><title>作業完了報告</title><style>body{font-family:sans-serif;max-width:900px;margin:40px auto;line-height:1.8}th,td{border:1px solid #ccc;padding:8px}table{border-collapse:collapse;width:100%}</style></head><body><h1>Modern Web Guidance レビュー指摘 作業完了報告</h1><p>対象: F-1〜F-10（F-11/F-12は対象外）</p><table><tr><th>検証</th><th>結果</th></tr><tr><td>Vitest</td><td>562 PASS</td></tr><tr><td>Playwright E2E</td><td>424 PASS / 8 skip / 432 total</td></tr><tr><td>エビデンス</td><td>${results.length} checks / ${fails.length} failures</td></tr></table><p>PC/iPad/iPhoneの公開サイトと、OAuth・GitHub APIモックで認証したCMS画面を確認した。</p></body></html>`);
