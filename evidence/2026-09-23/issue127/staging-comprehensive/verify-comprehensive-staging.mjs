/**
 * 包括的E2Eエビデンス取得スクリプト（2026-09-23, Issue #127 退行確認）
 * 雛形: evidence/2026-05-24/verify-comprehensive.mjs（2026-09-20 版をコピー）
 *
 * Issue #127 での変更点:
 *   - config.yml に base_url / branch が無くなり、base_url は location.origin から導出されるため、
 *     dist/admin/config.yml の一時書き換え（旧: base_url → localhost）を削除した。書き換えなしでログインできることが退行確認の一部。
 *   - 出力先を evidence/2026-09-23/issue127/comprehensive/ に変更。ポートを 4277 に変更（他作業のサーバーと衝突回避）。
 *
 * 検証項目:
 ...
 * 使用方法:
 *   1. npm run build
 *   2. node evidence/2026-09-23/issue127/comprehensive/verify-comprehensive.mjs
 */
...
const REMOTE_SITE = !/^http:\/\/localhost(?::|\/)/.test(BASE_URL);
const TODAY = '2026-09-23';
const EVIDENCE_DIR = process.env.E2E_EVIDENCE_DIR
  ? resolve(process.env.E2E_EVIDENCE_DIR)
  : join(process.cwd(), 'evidence', TODAY, 'issue127', 'staging-comprehensive');
...
<p>日付: ${TODAY} | 対象URL: ${BASE_URL} | staging実配信（OAuth/GitHub APIはモック） | デバイス: PC / iPad Pro 11 / iPhone 14</p>
...
async function main() {
  ...
  if (REMOTE_SITE) {
    ...
    if (!response.ok) throw new Error(`実配信サイト ${BASE_URL} がHTTP ${response.status}`);
    console.log(`🌐 staging実配信サイト検証: ${BASE_URL}`);
  } else {
    const distDir = join(process.cwd(), 'dist');
    configPath = join(distDir, 'admin', 'config.yml');
    configBefore = readFileSync(configPath, 'utf-8');
    if (/^\s*(base_url|branch)\s*:/m.test(configBefore)) {
      throw new Error('dist/admin/config.yml に base_url / branch が残っている（Issue #127 以前のビルド）');
    }
  ...
  try {
    ...
    await browser.close();
    server?.close();
    // Issue #127: config.yml は一度も書き換えていないことを確認（ローカルのみ）
    if (configPath && readFileSync(configPath, 'utf-8') !== configBefore) {
      console.error('dist/admin/config.yml が変更されている');
      process.exitCode = 1;
    }
  }
