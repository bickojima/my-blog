...
const EVIDENCE_DIR = join(process.cwd(), 'evidence', TODAY, 'issue127', 'comprehensive');
...
async function main() {
  ...
  // Issue #127: config.yml は書き換えない（base_url / branch は admin/index.html が location から導出する）。
  // 書き換え前提が残っていないことを確認する。
  ...
  try {
    ...
    await browser.close();
    server.close();
    // Issue #127: config.yml は一度も書き換えていないことを確認
    if (readFileSync(configPath, 'utf-8') !== configBefore) {
