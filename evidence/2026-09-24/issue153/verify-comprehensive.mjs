...
const EVIDENCE_DIR = join(process.cwd(), 'evidence', TODAY, 'issue153');
...
const ARTICLES = [
  { path: 'src/content/posts/2026/02/ramen.md',   sha: 'a1', title: 'ラーメン日記',    date: '2026-02-24', draft: false, tags: ['グルメ', '日常'] },
  { path: 'src/content/posts/2026/02/astro.md',   sha: 'a2', title: 'Astroの始め方', date: '2026-02-20', draft: true,  tags: ['技術'] },
  { path: 'src/content/posts/2026/01/newyear.md', sha: 'a3', title: '新年のご挨拶',   date: '2026-01-15', draft: false, tags: ['雑記'] },
  { path: 'src/content/posts/2025/12/yearend.md', sha: 'a4', title: '年末のまとめ',   date: '2025-12-31', draft: false, tags: ['雑記'] },
];

// モック固定ページ（2件）
const PAGES = [
  { path: 'src/content/pages/about.md',   sha: 'p1', title: 'このサイトについて', slug: 'about',   order: 1, draft: false },
  { path: 'src/content/pages/profile.md', sha: 'p2', title: 'プロフィール',       slug: 'profile', order: 2, draft: false },
];
...
(function(){
  ...
  await context.route('**/repos/bickojima/my-blog/git/**', (route) => {
    ...
    if (url.includes('/trees')) {
      ...
      let treeEntries;
      if (url.includes('src/content/posts')) {
        const prefix = url.match(/src\/content\/posts(?:\/[^?]+)?/)?.[0] || 'src/content/posts';
        treeEntries = ARTICLES.filter(f => f.path.startsWith(`${prefix}/`)).map(f => ({
          path: f.path.slice(prefix.length + 1), mode: '100644', type: 'blob', sha: f.sha,
        }));
      } else if (url.includes('src/content/pages')) {
        const prefix = url.match(/src\/content\/pages(?:\/[^?]+)?/)?.[0] || 'src/content/pages';
        treeEntries = PAGES.filter(f => f.path.startsWith(`${prefix}/`)).map(f => ({
          path: f.path.slice(prefix.length + 1), mode: '100644', type: 'blob', sha: f.sha,
        }));
      } else {
...
async function addRedBorder(page, selector, label) {
  return page.evaluate(({ s, l }) => {
    const el = [...document.querySelectorAll(s)].find((candidate) => {
      const rect = candidate.getBoundingClientRect();
      return Boolean(rect.width && rect.height && rect.bottom > 0 && rect.top < innerHeight && rect.right > 0 && rect.left < innerWidth && candidate.getClientRects().length);
    });
    if (!el) return false;
...
async function addRedBorderAt(page, rect, label) {
  if (!rect) return false;
  return page.evaluate(({ box, text }) => {
    const overlay = document.createElement('div');
    overlay.className = '_rbOverlay';
    overlay.style.cssText = `position:fixed;top:${box.y - 3}px;left:${box.x - 3}px;width:${box.width + 6}px;height:${box.height + 6}px;border:3px solid red;border-radius:4px;z-index:99999;pointer-events:none;box-sizing:border-box;`;
    const badge = document.createElement('div');
    badge.style.cssText = 'position:absolute;top:-22px;left:0;background:red;color:white;font-size:12px;padding:2px 6px;border-radius:2px;white-space:nowrap;';
...
async function installScreenshotAnnotations(page) {
  const originalScreenshot = page.screenshot.bind(page);
  page.screenshot = async (options = {}) => {
    const existing = await page.locator('._rbOverlay').count().catch(() => 0);
    if (existing === 0) {
      const selector = await page.evaluate(() => [
        '#cms-month-selector', '.entry-draft', '.entry-date', '#cms-public-url',
        '[data-slate-editor="true"]', 'input[id^="title-field"]',
        '[class*="GroupHeading"]', '[class*="ListCard"]', 'h1', 'main', 'body',
      ].find((candidate) => {
        return [...document.querySelectorAll(candidate)].some((element) => {
          const rect = element.getBoundingClientRect();
          return Boolean(rect.width > 0 && rect.height > 0 && rect.bottom > 0 && rect.top < innerHeight && rect.right > 0 && rect.left < innerWidth && element.getClientRects().length);
        });
      }) || 'body');
      const fileLabel = String(options.path || 'screenshot.png').split('/').pop().replace(/\.png$/i, '');
      await addRedBorder(page, selector, fileLabel);
    }
    try {
      return await originalScreenshot(options);
    } finally {
      await page.evaluate(() => document.querySelectorAll('._rbOverlay').forEach((node) => node.remove())).catch(() => {});
    }
  };
}
...
async function runCmsScenarios(page, deviceName, isMobile) {
  ...
  await (async () => {
    ...
    const entryHref = has ? await entry.getAttribute('href').catch(() => '') : '';
    if (has) {
      await entry.click();
      await page.waitForTimeout(3000);
    }
    await page.waitForFunction(() => {
      const title = document.querySelector('input[id^="title-field"]');
      const body = document.querySelector('[data-slate-editor="true"]');
      return title?.value && body?.textContent?.trim();
    }, undefined, { timeout: 10000 }).catch(() => {});
    const s = await page.evaluate((href) => ({
      hash: location.hash,
      entryHref: href,
      hasEditor: !!document.querySelector('[data-slate-editor], [class*="EditorContainer"], [class*="ControlBar"]'),
      title: document.querySelector('input[id^="title-field"]')?.value || '',
      body: document.querySelector('[data-slate-editor="true"]')?.textContent?.trim() || '',
    }), entryHref);
    const expectedEntryPath = ARTICLES[0].path.replace(/^src\/content\/posts\//, '').replace(/\.md$/, '');
    const pass = has && s.entryHref?.includes(expectedEntryPath) && s.hash.includes('entries') && s.title === ARTICLES[0].title && s.body.includes(`${ARTICLES[0].title}の本文テキスト`);
    const ss = `${id}-editor-${deviceName}.png`;
    if (s.title) await addRedBorder(page, 'input[id^="title-field"]', `タイトル読込: ${s.title}`);
    const previewBody = page.frameLocator('iframe').getByText(`${ARTICLES[0].title}の本文テキスト。`, { exact: true }).first();
    const previewBox = await previewBody.boundingBox().catch(() => null);
    if (previewBox) await addRedBorderAt(page, previewBox, '本文プレビュー');
    await page.screenshot({ path: join(SCREENSHOT_DIR, ss), fullPage: false });
    record({ device: deviceName, id, name: '記事エディタ開く（実クリック・既存タイトル/本文読込）', pass, screenshot: `screenshots/${ss}`, detail: s });
  })();
  ...
  await (async () => {
    ...
    await addRedBorder(page, 'label', '新規記事フォーム: タイトル');
    await page.screenshot({ path: join(SCREENSHOT_DIR, ss), fullPage: false });
  ...
  await (async () => {
    ...
    await addRedBorder(page, 'label', '固定ページフォーム: タイトル');
    await page.screenshot({ path: join(SCREENSHOT_DIR, ss), fullPage: false });
  ...
  await (async () => {
    ...
    const has = await entry.isVisible().catch(() => false);
    if (has) {
      const entryHref = await entry.getAttribute('href').catch(() => '');
      let clickError = null;
      try {
      ...
      if (!clickError) await page.waitForFunction(() => {
        const title = document.querySelector('input[id^="title-field"]');
        const body = document.querySelector('[data-slate-editor="true"]');
        return title?.value && body?.textContent?.trim();
      }, undefined, { timeout: 10000 }).catch(() => {});
      ...
      const expectedEntryPath = ARTICLES[0].path.replace(/^src\/content\/posts\//, '').replace(/\.md$/, '');
      const pass = !clickError && entryHref?.includes(expectedEntryPath) && s.hash.includes('entries') && s.title === ARTICLES[0].title && s.body.includes(`${ARTICLES[0].title}の本文テキスト`);
      ...
      if (previewBox) await addRedBorderAt(page, previewBox, '本文プレビュー');
      await page.screenshot({ path: join(SCREENSHOT_DIR, ss), fullPage: false });
      record({ device: deviceName, id, name: '記事エントリー実クリック→既存タイトル/本文読込', pass, screenshot: `screenshots/${ss}`, detail: { ...s, entryHref, clickError } });
      const back = page.locator('[class*="BackCollection"], a:has-text("記事")').first();
      if (await back.isVisible().catch(() => false)) { await back.click(); await page.waitForTimeout(2000); }
    } else {
...
<title>包括的E2Eエビデンスレポート ${TODAY}</title>
<style>
...
<body>
<h1>📋 包括的E2Eエビデンスレポート</h1>
<p>日付: ${TODAY} | ブランチ: staging | デバイス: PC / iPad Pro 11 / iPhone 14</p>
<p>シナリオ: S01〜S10（公開サイト）/ T01〜T55（CMS管理画面・探索）/ 合計${scenarios.length}シナリオ × 3デバイス</p>
...
<table>
  ...
  writeFileSync(join(EVIDENCE_DIR, 'comprehensive-results.json'), JSON.stringify({
    date: TODAY, summary: { total, passCount, failCount, pct },
    bugs: bugCandidates, results,
  }, null, 2));
  console.log(`\n📄 レポート保存: ${EVIDENCE_DIR}/report.html`);
}
...
async function main() {
  ...
  try {
    for (const device of DEVICES_CONFIG) {
      ...
      const isMobile = device.name !== 'PC';

      // サイトテスト
      {
        const ctx  = await browser.newContext({ ...device.config });
        const page = await ctx.newPage();
        await installScreenshotAnnotations(page);
        await runSiteScenarios(page, device.name);
        await ctx.close();
      }
      ...
      // CMSテスト
      {
        ...
        const page = await ctx.newPage();
        await installScreenshotAnnotations(page);
        await runCmsScenarios(page, device.name, isMobile);
        await ctx.close();
      }
    }
  } finally {
