...
## 12. 管理画面HTML検証 (`admin-html.test.mjs`) — 56件
...
現在のテストは静的解析・単体テスト・ビルド成果物検証で構成されている。以下の検証は現行テストでは実施できない。

| 検証できない項目 | 理由 |
| :--- | :--- |
| CMS管理画面のブラウザ操作（記事作成→保存→公開） | ブラウザ自動操作環境が未導入である |
| OAuth認証の実際のフロー（ポップアップ→GitHub→コールバック） | 実際のGitHub OAuth連携が必要である |
| モバイル端末での表示・操作 | 実機またはエミュレータが必要である |
| 記事編集→ビルド→閲覧の一気通貫テスト | CMSからのGitHub commit操作が含まれるため |

### 14.2 E2Eテスト導入の提案

動的操作を想定したテストを導入する場合、以下のアプローチが考えられる。

#### 案1: Playwright によるブラウザ自動操作テスト（推奨）
...
| 対象ブラウザ | Chromium, Firefox, WebKit（iOS Safari相当） |
| テスト可能項目 | ページ遷移、リンク確認、レスポンシブ表示、アーカイブナビ操作 |
| 導入コスト | 中（パッケージ追加 + テストスクリプト作成） |
| CI実行 | Cloudflare Pages のビルド前に `npx playwright test` で実行可能 |

想定テストケース:

| No. | テストケース | 検証内容 |
| :--- | :--- | :--- |
| E-01 | トップページに記事一覧が表示される | `npm run dev` → `http://localhost:4321` にアクセスし記事リンクが描画されること |
| E-02 | 記事リンクをクリックすると記事ページに遷移する | 記事リンクclickで `/posts/yyyy/mm/slug` に遷移し記事タイトルが表示されること |
| E-03 | タグリンクをクリックするとタグ一覧に遷移する | タグclickで `/tags/xxx` に遷移し該当記事が表示されること |
| E-04 | アーカイブナビから年月ページに遷移できる | ArchiveNavのリンクclickで年別・月別ページに遷移すること |
| E-05 | モバイル画面幅（375px）で記事一覧が正常に表示される | viewport=375pxで表示崩れがないこと（スクリーンショット比較） |
| E-06 | 管理画面（/admin/）が読み込める | `/admin/` にアクセスし Decap CMS のUIが描画されること |

#### 案2: CMS操作テスト（GitHub OAuth モック）

CMS管理画面の操作テストには、OAuth認証をモックしてDecap CMSの操作をPlaywrightで自動化する方法がある。ただし、Decap CMSはReactベースのSPAであり、DOM構造がバージョンにより変化するためメンテナンスコストが高い。

| 項目 | 内容 |
| :--- | :--- |
| 実現難度 | 高 |
| メンテナンスコスト | 高（Decap CMSのDOM構造に依存） |
| 推奨 | 当面は手動テストで代替する |

### 14.3 現時点の判断

E2Eテスト（案1: Playwright）は閲覧側のページ遷移・レスポンシブ表示の検証に有効である。ただし、現状のテストカバレッジ（227件、FR-10を除く全要件充足）で品質は十分に担保されているため、優先度は「将来的な改善」とする。CMS操作テスト（案2）は費用対効果の観点から当面見送り、手動テストで代替する。
...
| `admin-html.test.mjs` | 56 | PASS | 6ms |
| `rehype-image-caption.test.mjs` | 8 | PASS | 2ms |
| `auth-functions.test.mjs` | 10 | PASS | 26ms |
| `content-validation.test.mjs` | 95 | PASS | 41ms |
| `build.test.mjs` | 31 | PASS | 1450ms |
| **合計** | **227** | **全PASS** | **1528ms** |
